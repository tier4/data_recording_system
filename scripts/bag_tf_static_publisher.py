...
from rosbag2_py import StorageFilter, SequentialReader, StorageOptions, ConverterOptions
...
class BagTFStaticPublisher(Node):
    def __init__(self, bag_path):
        ...
        self.tf_static_publisher = self.create_publisher(
            TFMessage,
            '/tf_static',
            static_broadcaster_qos
        )

        self.get_logger().info(f'Publishing {len(self.tf_static_data.transforms)} static transforms from bag')
        ...
        try:
            ...
            while reader.has_next():
                ...
                if topic == '/tf_static':
                    ...
                    self.get_logger().info(f'Found /tf_static with {len(tf_static_data.transforms)} transforms')
...
def main(args=None):
    parser = argparse.ArgumentParser(
        description='Publish /tf_static from a ROS2 bag file',
...
Example usage:
  ...
  %(prog)s /path/to/your/bag.mcap
    ...
    parser.add_argument(
        'bag_path',
        help='Path to the ROS2 bag file (MCAP format)'
    )
